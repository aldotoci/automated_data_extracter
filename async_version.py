...
class store_info_in_database:
    ...
    async def store_data_in_database(self,a, db,prog):
        try:
            print(prog)
            #Getting info
    ...
    async def get_anime_watch_url(self):
        ...
        for a in range(1,10801):
            ...
            #t.start()
            prog += 1
            task = asyncio.create_task(self.store_data_in_database(a,db,prog))


test = store_info_in_database()
asyncio.run(test.get_anime_watch_url())